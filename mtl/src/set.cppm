...
public:
    ...
    size_t erase(const T& elem) { return static_cast<size_t>(tree_.remove(elem)); }

    iterator erase(iterator itr) { return tree_.remove(itr); }

    iterator find(const T& elem) { return tree_.find(elem); }
    const_iterator find(const T& elem) const { return tree_.find(elem); }

    [[nodiscard]] bool contains(const T& elem) const { return tree_.contain(elem); }
};
